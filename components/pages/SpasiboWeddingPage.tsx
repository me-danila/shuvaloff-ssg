...
const copyByLocale: Record<Locale, SpasiboCopy> = {
    ru: {
        thanks: "спасибо за обращение!",
        contactMessage: "Мы свяжемся с Вами в ближайшее время",
        backButton: "Вернуться назад",
    },
    en: {
        thanks: "thank you for contacting us!",
        contactMessage: "We will contact you shortly",
        backButton: "Go back",
    },
};

export default function SpasiboWeddingPage({ locale }: { locale: Locale }) {
    const copy = copyByLocale[locale];

    return (
        <main className="flex flex-col gap-8 -mb-12 xl:-mb-20">
            <section>
                {/* Фото с подписью */}
                    ...
                    <StaggerContainer className="flex gap-2 h-65 xl:h-180 xl:gap-4 xl:w-full">
                        <div className="absolute bottom-10 md:bottom-20 xl:bottom-32 inset-x-0 text-center text-white z-10 flex flex-col gap-3 px-10 xl:max-w-2xl xl:mx-auto xl:gap-6">
                            <StaggerItem className="relative flex-1 min-w-0 overflow-hidden rounded-lg xl:rounded-md">
                                <h1 className="text-4xl xl:text-5xl uppercase">
                                    {copy.thanks}
                                </h1>
                            </StaggerItem>
                            <StaggerItem className="relative flex-1 min-w-0 overflow-hidden rounded-lg xl:rounded-md">
